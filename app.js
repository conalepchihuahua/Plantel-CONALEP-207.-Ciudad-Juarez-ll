...
function simular(alumnos,fechas){
  ...
  // Assign a unique ID to each team for tracking counts during simulation
  ...
  while(d<=h){
    ...
    if(dow!==0&&dow!==6&&!cancelado(d)){
      ...
      let eligible=teams.filter(t=>!restr||!t.tieneMMFD);
      if(eligible.length>0){
        // Find team with the lowest count among eligible
        let minCount=Infinity;
        eligible.forEach(t=>{const c=counts.get(t.id);if(c<minCount)minCount=c;});
        
        // From those with the lowest count, we want the one that has been waiting the longest.
        // To approximate queue logic, we can just take the first one we encounter (since teams array is ordered).
        let chosen=eligible.find(t=>counts.get(t.id)===minCount);
        
        if(set.has(fkey(d)))result.set(fkey(d),chosen.miembros);
...
async function _registrarAusencia(fbKey,dateKey,aus,desc,ico,btn,btnIco,btnLbl,list){
  ...
  try{
    ...
    // Notificar al alumno inmediatamente
...
function mkTurnStrip(cont,fecha,equipo,yoIdx){
  ...
  el.innerHTML=`<div class="my-turn-info"><p class="my-turn-label">Tu próxima asignación</p><p class="my-turn-date">${fstr(fecha)}</p><p class="my-turn-sub"><i class="ph ph-users"></i>${equipo.length} integrantes &bull; <i class="ph ph-clock-counter-clockwise"></i>${limpiezas} ${limpiezas===1?'turno registrado':'turnos registrados'}</p></div><i class="ph ph-caret-right my-turn-chevron"></i>`;
...
function mkCard(cont,equipo,fecha,titulo,esCancelado,admin){
  ...
  const limpiezas=_conteosEquipos.get(k)||0;
  info.innerHTML=`<p class="card-date">${fstr(fecha)}</p><div class="card-title-row" style="flex-wrap:wrap;gap:6px"><p class="card-title" style="flex:1 1 100%">${titulo}</p><span class="card-count"><i class="ph ph-users"></i>${equipo.length} integrantes</span><span class="card-count"><i class="ph ph-clock-counter-clockwise"></i>${limpiezas} ${limpiezas===1?'turno registrado':'turnos registrados'}</span></div>`;
...
function abrirOrdenSheet(){
  ...
  teams.forEach((team,i)=>{
    ...
    const fbKeys=team.miembros.map(a=>_fbKeyMap.get((a['Correo Institucional']||'').toLowerCase())||'').filter(Boolean);
...
async function _guardarOrden(body){
  ...
  keys = [...new Set(keys)]; // prevent duplicates
  if(!keys.length)return;
  try{
    await db.ref('ordenAlumnos').set(keys);
    _ordenPersonalizado=keys;
    const keyMap=new Map(_alumnos.map(a=>[_fbKeyMap.get((a['Correo Institucional']||'').toLowerCase()),a]));
...
async function cargarAseo(){
  ...
  try{
    ...
    try{
      ...
      let orden=ordenSnap.val();
      if(Array.isArray(orden)&&orden.length){
        orden = [...new Set(orden)]; // prevent corrupted data
